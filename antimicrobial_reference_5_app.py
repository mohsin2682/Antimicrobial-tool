...
ANTIMICROBIALS = {
    ...
    "ciprofloxacin": {
        ...
        "side_effects": "⚠️ Tendon rupture (Achilles). Peripheral neuropathy. CNS agitation. QT prolongation. May exacerbate myasthenia gravis (fluoroquinolone-class boxed warning).",
    ...
    "levofloxacin": {
        ...
        "side_effects": "⚠️ Tendon rupture, peripheral neuropathy, CNS effects, QT prolongation, may exacerbate myasthenia gravis. Same class warnings as ciprofloxacin.",
        "pregnancy": "Category C. Avoid in pregnancy/breastfeeding.",
        "cross_allergy": "No cross-reactivity with PCN.",
    ...
    "penicillin_g": {
        ...
        "dosing_adults": "IV: 2-4 million units q4h (12-24 million units/day for severe infection). IM (benzathine, for syphilis): 2.4 million units single dose for primary/secondary/early-latent syphilis — late-latent or unknown-duration syphilis requires 2.4 million units IM weekly x3 doses instead (a single dose under-treats).",
    ...
    "moxifloxacin": {
        ...
        "side_effects": "🚨 QT prolongation (more than other fluoroquinolones — avoid with other QT-prolonging drugs), tendon rupture, peripheral neuropathy, CNS effects, aortic aneurysm/dissection risk, may exacerbate myasthenia gravis (avoid in known MG — part of the fluoroquinolone-class boxed warning).",
        "pregnancy": "Category C. Avoid in pregnancy/breastfeeding.",
        "cross_allergy": "No cross-reactivity with PCN.",
    ...
    "ofloxacin": {
        ...
        "side_effects": "🚨 Tendon rupture, peripheral neuropathy, CNS agitation, QT prolongation, photosensitivity, may exacerbate myasthenia gravis (avoid in known MG — part of the fluoroquinolone-class boxed warning).",
        "pregnancy": "Category C. Avoid in pregnancy/breastfeeding.",
        "cross_allergy": "No cross-reactivity with PCN.",
    ...
    "minocycline": {
        ...
        "dosing_renal": "No adjustment typically needed.",
        "pharmacist_notes": "⚠️ Vestibular side effects (dizziness, ataxia) are more common than with doxycycline — counsel patients, especially those driving/operating machinery. Note: this Watch classification is for oral minocycline per WHO's 2023 AWaRe list; the (rarely used) IV formulation is separately classified Reserve.",
    },
    "colistin": {
    ...
    "fosfomycin": {
        ...
        "pharmacist_notes": "⚠️ Oral fosfomycin trometamol (for UTI, Watch-classified) is NOT the same product or dose as IV fosfomycin disodium (Reserve-classified, used for severe MDR infections) — the two must not be confused.",
    },
    "amikacin": {
    ...
    "itraconazole": {
        ...
        "side_effects": "🚨 Boxed warning — negative inotrope: should NOT be given for onychomycosis in patients with ventricular dysfunction/heart failure (contraindicated, not just cautioned), and must be stopped immediately if signs of CHF develop during any indication. Also: hepatotoxicity, GI upset, significant CYP3A4 drug interactions, erratic/formulation-dependent oral absorption.",
...
with st.sidebar:
    ...
    with st.expander(f"📇 Browse all {len(ANTIMICROBIALS)} drugs by class", expanded=False):
        st.caption("A second way to find a drug — pick a class heading, then click a name to jump straight to it, alongside the search dropdown in the main panel.")
        _classes_grouped = {}
        for _k in sorted(ANTIMICROBIALS, key=label_for):
            _classes_grouped.setdefault(ANTIMICROBIALS[_k]["drug_class"], []).append(_k)
        _class_labels = [f"{_cls} ({len(_ks)})" for _cls, _ks in sorted(_classes_grouped.items())]
        _class_lookup = dict(zip(_class_labels, sorted(_classes_grouped)))
        _picked_label = st.selectbox("Drug class heading", _class_labels, key="browse_class_pick")
        _picked_class = _class_lookup[_picked_label]
        for _k in _classes_grouped[_picked_class]:
